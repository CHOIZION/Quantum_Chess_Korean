...
class ChessUtils:
    ...
    def queenMoveValid(position, move):
        ...
        if abs(movementInCols) == abs(movementInRows):
    ...
    def moveHint(position, flags):
        ...
        if position.isQuantic():
            ...
            return ChessUtils.optimize(position, flags, averageEvalForMoves, list(list(zip(*listAllMoves))[0]))
        else:
            try:
                fen = position.boardToFen(flags)
                stockfish.set_fen_position(fen)
